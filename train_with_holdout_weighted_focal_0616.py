...
# [3] Focal Loss with dynamic alpha
...
class MaskedFractureDataset(UnifiedFractureDataset):
    def __getitem__(self, idx):
        ...
        return mask_side_markers(image), label

# [5] Fixed 120-image Test Set
def extract_holdout_test_set(df):
    ...
    for source, (n_pos, n_neg) in {
        ...
        test_parts.append(pd.concat([
            df_src[df_src['label'] == 1].sample(n=n_pos, random_state=42),
            df_src[df_src['label'] == 0].sample(n=n_neg, random_state=42)
        ]))
...
def train(trainval_df):
    mlflow.start_run(run_name="train_weighted_focal")
    ...
    alpha = neg / (pos + neg)
    print(f"🔧 Using FocalLoss(alpha={alpha:.4f}, gamma=2.0)")
    criterion = FocalLoss(alpha=alpha, gamma=2.0)

    # Optional sampler (not required if using alpha)
    class_weights = 1.0 / torch.tensor([neg, pos], dtype=torch.float)
...
if __name__ == "__main__":
    ...
    trainval_df, test_df = extract_holdout_test_set(df)
    train(trainval_df)
