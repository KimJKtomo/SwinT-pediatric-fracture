...
def get_dataset_paths(base_path, part):
    if part == 'radius':
        return os.path.join(base_path, 'segmentation_frac', 'radius_set', 'train')
    elif part == 'scaphoid':
        return os.path.join(base_path, 'segmentation_frac', 'scaphoid_set', 'All_image')
    elif part == 'styloid':
        return os.path.join(base_path, 'segmentation_frac', 'styloid_set', 'All_image')
    else:
...
def load_mura():
    ...
    train_img_df['study_id'] = train_img_df['path'].apply(lambda x: '/'.join(x.split('/')[:5]))  # includes study1_positive
    ...
    # Update actual image path
    train_df["image_path"] = train_df["path"].apply(lambda x: os.path.join("/mnt/data/KimJG/ELBOW_test", x))

    train_df["part"] = "wrist"  # or detect from path
    ...
    # Optional: filter only wrist X-rays
    train_df = train_df[train_df['path'].str.contains("XR_WRIST")]

    if len(train_df) < 10:
        print("⚠️ Warning: MURA dataset has very few XR_WRIST samples.")

    # 🔀 Split
    train_mura, val_mura = train_test_split(train_df, test_size=0.2, stratify=train_df['label'], random_state=42)
    train_mura['split'] = 'train'
    val_mura['split'] = 'val'

    return pd.concat([train_mura, val_mura], ignore_index=True)


def get_combined_dataset_v3(image_only=True, fracture_only=True):
    ...
    train_kaggle, val_kaggle = train_test_split(train_val_df, test_size=0.2, stratify=train_val_df['label'], random_state=42)
    train_kaggle['split'] = 'train'
    val_kaggle['split'] = 'val'
    kaggle_df = pd.concat([train_kaggle, val_kaggle, test_df], ignore_index=True)
    ...
    hospital_train, hospital_val = train_test_split(hospital_df, test_size=0.2, stratify=hospital_df['label'], random_state=42)
    hospital_train['split'] = 'train'
    hospital_val['split'] = 'val'
    hospital_df = pd.concat([hospital_train, hospital_val], ignore_index=True)
